...
**HARD gate:** `$ARTIFACTS/idea/prd.md` must exist. If missing: STOP. Tell the user to complete the idea phase first (`/sdlc:product-spec` or `/sdlc:idea`).

**Soft check:** `$ARTIFACTS/journey/customer-journey.md` is preferred. If missing: warn the user that journey mapping enriches the screen inventory, recommend running `/sdlc:customer-journey` first, but allow continuation.
...
Ready to continue?
  → Proceed to data model:  /sdlc:data-model
