...
## Verification Stack (runs after every file write)

Configure a PostToolUse hook that runs after every Edit/Write tool call during this phase. The stack runs in this order — stop at first failure:

1. **Lint** — fix auto-fixable issues, fail on remaining errors
   - TS/JS: `npx eslint --fix <file> && npx eslint <file>`
   - Python: `ruff check --fix <file> && ruff check <file>`
...
2. **Format** — enforce consistent style
   ...
   - Go: (gofmt above handles format)

3. **Type check** — catch type errors immediately (not at build time)
   - TS: `npx tsc --noEmit`
   - Python: `mypy <file> --ignore-missing-imports`

4. **Unit tests for the changed file** — run only tests related to the changed file for speed
   - Jest: `npx jest --testPathPattern=<test-file-pattern> --passWithNoTests`
   - pytest: `pytest tests/unit/ -k <module-name> --no-header -q`
   - Go: `go test ./... -run <TestName>`

**Failure handling:**
- If lint/format fails → auto-fix and retry once. If still failing: surface specific error to user, mark task BLOCKED.
- If type check fails → fix the type error before continuing with any other file writes.
- If unit test fails → this is expected during TDD (the TDD stub will fail until implemented). Once implementation is complete, all tests must pass before task is marked done.
- If unit tests were passing and now fail after a change → STOP immediately. Do not proceed to the next task. Fix the regression before continuing.

**TDD reminder:** Tests generated by bdd-tdd-scaffold are designed to FAIL initially. The implementation cycle is:
1. Confirm stub test fails (expected)
2. Implement the feature
3. Confirm stub test now passes
...
For complex implementation tasks, these subagent roles can be invoked to parallelize or specialize work. Each has a minimal tool set to reduce risk.

**code-architect** — designs implementation approach before writing code
- Tools: Read, Glob, Grep (read-only)
- Prompt: "Given [context], design the implementation approach for [task]. Output: file list to create/modify, function signatures, data flow. Do NOT write any code."

**code-simplifier** — reviews written code for unnecessary complexity
- Tools: Read, Glob, Grep (read-only)
- Prompt: "Review [file]. Identify: over-engineering (abstractions for one-time use), premature optimization, unnecessary dependencies. Output: specific simplifications with before/after."

**security-reviewer** — reviews implementation for security issues before merging
- Tools: Read, Glob, Grep (read-only)
- Prompt: "Review [file] against the threat model at $ARTIFACTS/design/threat-model.md. Check: input validation, auth checks, SQL injection, output encoding. Output: findings with severity and specific line references."

**test-validator** — validates that implementation matches test cases
- Tools: Read, Glob, Grep, Bash (test runner only)
- Prompt: "Check that [implementation file] correctly implements the behavior specified in [TC-IDs]. Run the relevant tests. Output: pass/fail per TC-ID with explanation."

Use these for tasks marked COMPLEX in implementation-plan.md, or any task touching auth, payments, or PII handling.
