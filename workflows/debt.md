...
Run /sdlc:start to initialize a project first.
```
STOP.
...
To add items:
  /sdlc:debt --add "<description of the debt>"

To discover debt automatically:
  Run /sdlc:gaps — spawns 3 analysis agents and populates the register.
```
...
If description is empty: output "Please provide a description. Example: /sdlc:debt --add 'UserService is a 600-line fat class that mixes domain logic with HTTP handling'" STOP.
...
**Validate:**
- TD-ID must exist in technicalDebts array. If not: output "TD-ID '<id>' not found in the register. Run /sdlc:debt to see all IDs." STOP.
...
---
*Generated by /sdlc:debt — re-run to refresh*
```
...
```
  ! <N> CRITICAL items require immediate attention.
    ...
    Run /sdlc:gaps for root-cause analysis of debt patterns.
```

If no items exist:
```
    Debt register is clean. Run /sdlc:gaps periodically to catch new debt.
```

Otherwise:
```
    Run /sdlc:debt --add "<description>" to add items manually.
    Run /sdlc:debt --resolve <TD-ID> to mark items fixed.
    Run /sdlc:debt --export to generate a shareable debt report.
    Run /sdlc:gaps to auto-discover debt via codebase analysis.
```
